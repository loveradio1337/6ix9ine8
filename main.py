...
async def on_message(message):
        ...
        if message.content.startswith(">stoprainbow"):
                await client.send_message(message.channel,"I sucessfully stopped it.")
                try:
        ...
        if message.content.startswith(">rainbow"):
                ...
                hue=0
                if message.content.strip().startswith(">rainbow "):
                        ...
                        while dothething[str(message.server.id)]:
                                ...
                                if max(users):
                                        for i in range(50): #arbitrary rate limits
                                                ...
                                                rol = discord.Colour(((rgb[0]<<16) + (rgb[1]<<8) + rgb[2]))
                                                try:
                                                        await client.edit_role(message.server, role, colour=rol)
                                                except:
